...
- **next_6:** Define the Universal Production Blueprint as a concrete document schema that assembles all five existing documents (Principle Kernel, Narrative/Emotional, Musical, Cinematic, Editorial) into the single package described in the mission.
- **next_7:** Provider-Specific Production Package: the translation layer from the abstract Universal Production Blueprint into one real provider's prompt format (start with one video provider and one music provider, not all five/two).
