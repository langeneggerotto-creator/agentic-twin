...
| Provider-neutral abstract production spec + per-provider translation | IMPLEMENTED for one video provider (Runway) and one music provider (Suno) — see Provider-Specific Production Package below. Not yet built for the other 4 video / 1 music candidates. |
...
| Character Performance Package schema (character-consistent reference images + characters singing instead of narration) | IMPLEMENTED — `schemas/character_performance_package.schema.json` + `schemas/character_performance_package.py` + `tests/test_character_performance_package.py` (10 passing smoke tests). See "Characters singing themselves, not a narrator" above. `no_narrator` and `lip_sync_required` are both literal hard gates (`const: true`), not descriptions. Worked example: The Builder and The Asker cover all 8 Hope-chain shots between them, using Nano Banana as the character-reference provider. |
