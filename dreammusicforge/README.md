...
- **next_7:** Provider-Specific Production Package: the translation layer from the Universal Production Blueprint's `provider_specific_prompt_sets` (currently `build_status: "not_yet_built"`) into one real provider's prompt format — start with one video provider and one music provider, not all five/two.
...
No dependencies beyond the Python 3 standard library. The last test is the closest thing to a full pipeline integration test — it re-validates all five prior documents and their cross-document links in one run.
