...
- **next_4:** Define the Universal Production Blueprint as a concrete document schema (even if most fields start as free text) so "complete package" has one canonical shape.
- **next_5:** Cinematic Architecture Engine schema (shot-level camera/lighting/blocking design), which can now cross-reference both the Narrative + Emotional Architecture and the Musical Architecture's tempo map the same way Musical Architecture cross-references Narrative.
