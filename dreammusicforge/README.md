...
| Assembly Package schema (native audio-driven generation requirement + duration reconciliation + ffmpeg stitching plan) | IMPLEMENTED — `schemas/assembly_package.schema.json` + `schemas/assembly_package.py` + `tests/test_assembly_package.py` (17 passing smoke tests). See "Native audio-driven generation and post-generation assembly" above. `reconciliation_strategy`, `assembly_status`, and `concat_command` are all reproducibility-checked, not just asserted. Worked example deliberately routes one of 8 cuts to `requires_manual_review` (39% simulated drift) so `assembly_status` demonstrably blocks rather than rubber-stamping. |
...
- **next_13:** Live API integration layer — real credentials (stored outside the repo, never committed), real HTTP calls to a video/music/character-reference provider, real error handling and rate-limit/retry logic, real `actual_duration_seconds` instead of the Assembly Package's simulated figures. This is the first stage that needs the user's own API keys and will incur real cost per call; deliberately not started until asked for by name, per this repo's own non-goals discipline against speculative infrastructure.
- **next_14 (candidate, not started):** Calibrate `generation_risk_estimate`'s per-shot success-probability range from real observed provider outcomes (an MCMC/Metropolis-Hastings posterior over provider reliability) instead of the currently-assumed uniform range. Deliberately not built yet — there is no observed data to calibrate against, and building the calibration machinery ahead of that data would be speculative infrastructure this repo's own non-goals discipline argues against.
